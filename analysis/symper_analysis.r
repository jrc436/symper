...
library(MuMIn)
symper_data <- read.csv(file="/work/research/symper/analysis/replication_run.csv")
...
model_all_good_features <- glmer(correct ~ T1_same + D1_same + same4fold + same3fold + (1|user_id) + (1|task_id), data=symper_data, family=binomial)
r.squaredGLMM(model_rot)
r.squaredGLMM(model_ref)
#matrix_dat <- read.csv(file="/home/jrc/noheadercont.csv", header=FALSE)
#mat = as.matrix(matrix_dat)
#mcnemar.test(mat)